...
MeshNode *create_node(const char *ip, int port) {
	MeshNode *node = (MeshNode *)malloc(sizeof(MeshNode));
	if (node == 0)
		return 0; /*! EXHAUSTED MEM*/

	strncpy((char *)node->ip, ip, ADDRESS);
	node->port = port;
	node->connected = false;
	node->node_packets = NULL;
...
__CJLF_GENERICS add_node(MeshGraph *graph, MeshNode *node) {
	...
	// release_lock(&global_lock_manager, graph->num_nodes);
}
...
void connect_nodes(MeshNode *a, MeshNode *b) {
...
void disconnect_nodes(MeshNode *a, MeshNode *b) {
...
void receive_message(MeshNode *node, Packet *pkt) {
	if (!node || !pkt)
		return;
